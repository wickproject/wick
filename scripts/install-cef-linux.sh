#!/bin/bash
# install-cef-linux.sh: Install the CEF renderer for Wick on Linux.
#
...
# source. Required once per host to enable JavaScript rendering.
#
# Usage (from a wick checkout):
#   sudo bash scripts/install-cef-linux.sh
#
# Or via `wick install cef`:
#   wick install cef    # prints the one-liner to run
#
# Installs to /opt/wick/ with everything CEF needs.
# For residential-IP tunnel setup, run: sudo wick-tunnel init
...
WRAPPER_PATH="/usr/local/bin/wick"

# Only replace the file if it's a Wick-managed symlink or file.
# Refuse to delete an unrelated binary that happens to share the name.
if [[ -e "$WRAPPER_PATH" || -L "$WRAPPER_PATH" ]]; then
    if [[ -L "$WRAPPER_PATH" ]]; then
        EXISTING_TARGET="$(readlink "$WRAPPER_PATH" || true)"
        if [[ "$EXISTING_TARGET" == "/opt/wick/"* || "$EXISTING_TARGET" == "$WICK_DIR/"* ]]; then
            rm -f "$WRAPPER_PATH"
        else
            echo "Refusing to remove existing symlink at $WRAPPER_PATH (target: $EXISTING_TARGET)." >&2
            echo "Please move or remove it manually, then rerun the installer." >&2
            exit 1
        fi
    else
        # Regular file — only remove if it looks like our wrapper script.
        if head -5 "$WRAPPER_PATH" 2>/dev/null | grep -q "wick-managed"; then
            rm -f "$WRAPPER_PATH"
        else
            echo "Refusing to overwrite existing file at $WRAPPER_PATH." >&2
            echo "Please move or remove it manually, then rerun the installer." >&2
            exit 1
        fi
    fi
...
#!/bin/bash
# wick-managed: generated by install-cef-linux.sh — do not edit.
...
if [[ -f /opt/wick/.wick-tz ]]; then
    ...
    _TZ=$(curl -fsS --interface 10.99.0.1 --connect-timeout 3 --retry 1 https://worldtimeapi.org/api/ip 2>/dev/null | grep -oE '"timezone":"[^"]*"' | cut -d'"' -f4)
    if [[ -n "$_TZ" ]]; then
